...
- **Currency**: USD is assumed throughout (`utils/helpers.parse_price` strips `$` and commas). eBay may render prices in other currencies depending on account/geo settings — parsing will break for non-`$`-prefixed formats.
- **eBay DOM volatility**: eBay changes markup and A/B-tests search-results templates. Locators are centralized as class constants on each page object; `search_results_page.py` includes a fallback to eBay's legacy list-based template if the current card-based template isn't found. All other locators (search box, price filter, variant dropdowns, add-to-cart, cart subtotal) may need adjustment if eBay changes its markup after this was written.
